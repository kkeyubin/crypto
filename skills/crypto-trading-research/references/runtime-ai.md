...
`bars`, and optional bid/ask, funding, and deterministic signal observations no
later than `cutoff`. Treat `cutoff` as the information boundary; do not infer or
use later market knowledge.
...
Emit only a schema-valid `AIAssessment`: UUID `assessment_id`, the supplied
`snapshot_id`, `SUPPORT`, `OPPOSE`, or `UNCERTAIN`, non-empty `reasons`,
non-empty `{skill, section}` citations, `risk_notes`, `market_data_cutoff`
equal to the snapshot `cutoff`, and `model_id`, `prompt_version`, and
`skill_version`. The closed schema admits no order, position, leverage, stop,
