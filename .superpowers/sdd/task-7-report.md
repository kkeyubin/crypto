...
- User actions receive monotonically increasing page-level focus tokens. Only the latest action may focus its recovery surface; moving focus outside that symbol cancels the token, and successful transfer to the replacement action consumes it. Required recovery, replacement-action, and confirmation initial/return handoffs use layout effects so focus is installed in the same commit before the user can observe the new surface. Background revalidation has no focus token, and slower earlier symbols cannot steal focus when they finish.
...
- `npm run web:test -- --run` — 4 files, 59 tests passed; the full command passed five consecutive post-fix runs with no flake (295 test executions total).
