...
19. Normal Overview/Symbols navigation conditionally unmounted the console, aborting a protected backfill request and losing its retry-only form state.
20. A full reload immediately replaced a ready dashboard with global loading and trusted a stale list over an already committed disable or enable mutation.
21. When one request inside a symbol evidence bundle failed, sibling requests sharing the operation signal were not aborted and could remain unresolved across retries.
...
26. Required recovery and replacement-action focus handoffs ran in passive effects, leaving a commit-to-effect window where the mounted recovery card was visible while focus remained on `body`; the full suite reproduced this race on the eighth consecutive pre-fix run.
27. A later test-only correction wrapped atomic focus assertions in `waitFor`, allowing a passive effect to repair focus before the assertion observed it instead of enforcing same-commit handoff.

Focused tests were observed failing for the missing behavior and then passing after each minimal correction. After moving the atomic focus handoffs to layout effects, both add/backfill race variants passed five consecutive targeted runs, followed by five consecutive complete Web-suite runs with no flake. The final focus-contract review restored synchronous `toHaveFocus()` checks after each awaited network/DOM transition. A temporary local mutation of all three focus layout effects back to passive effects then failed the focused `list-first` race on mutation run 3 with `body` still focused; restoring the production layout effects returned the same focused set to 6/6 passing.
...
- A ready dashboard stays mounted while a replacement list loads in the background. Only the winning list advances the generation and restarts dependent evidence, so a slow list does not remove an active recovery surface.
- Every evidence bundle shares one operation controller. Failure, invalid evidence, or timeout aborts that controller before the operation is cleared, cancelling still-pending sibling requests; a later retry starts with no live work from the failed operation.
...
- User actions receive monotonically increasing page-level focus tokens. Only the latest action may focus its recovery surface; moving focus outside that symbol cancels the token, and successful transfer to the replacement action consumes it. Required recovery, replacement-action, and confirmation initial/return handoffs use layout effects so focus is installed in the same commit before the user can observe the new surface. Tests wait only for the asynchronous network or DOM transition, then inspect `document.activeElement` synchronously; they do not grant a passive effect a second polling window. Background revalidation has no focus token, and slower earlier symbols cannot steal focus when they finish.
...
- The Symbols view is mounted lazily on first navigation, so Overview has no up-front symbol-data fan-out. After that first visit it remains mounted under a unique hidden `#symbols` container while inactive. The hidden subtree leaves the accessibility tree, but normal App navigation preserves protected form fields, in-flight backfill completion, and retry-only state without repeating symbol creation.
...
- `npm run web:test -- --run` — 4 files, 59 tests passed; the full command passed five consecutive runs after the layout-effect fix and a second five-run batch after synchronous focus assertions were restored, with no flake in either batch (295 test executions in the final batch).
...
- Protected add/backfill workflow state survives normal in-App navigation after the Symbols view is first mounted. It remains in-memory UI state and is not durable across a full browser reload or tab close.
